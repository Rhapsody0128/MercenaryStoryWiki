...
#army
  ...
  .row.p.mt
    ...
    v-btn.favorite(@click='clearFavoriteArmy()')
      span 清空所有收藏
  .row
    v-sheet.p.mt.rounded-lg(elevation="3" color='rgba(0,0,0,0)')
      soldierCard(v-for='data in selectHeros' :data='data' :favoriteTpye='favoriteType' :showHeart="true")
</template>
<script>
...
export default {
  ...
  mounted() {
    ...
    this.tag = JSON.parse(JSON.stringify(armyTagList));
    this.selectHeros = this.heros;
    let that = this;
